...
async function customerHandOff(tx) {
    if (tx.product.currentOwner === 'CUSTOMER' && tx.product.handoffTimestampHistory.length == 0) {
    ...
    } else if (tx.product.currentOwner !== 'CUSTOMER') {
        throw new Error('Unauthorized operation. This function can only be triggered once at the beginning by the customer.');
    } else if (tx.product.handoffTimestampHistory.length != 0) {
        throw new Error('Unauthorized operation. This operation has already been triggered in the past for this product.');
    }
}
...
async function retailerHandOff(tx) {
    if (tx.product.currentOwner === 'RETAIL' && tx.product.handoffTimestampHistory.length == 1) {
        ...
        let productRegistry = await getAssetRegistry(NS + '.Product');
        await productRegistry.update(tx.product);
    } else if (tx.product.currentOwner !== 'RETAIL') {
        throw new Error('Unauthorized operation. This function can only be triggered once by the retail store handler.');
    } else if (tx.product.handoffTimestampHistory.length < 1) {
        throw new Error('Unauthorized operation. This operation cannot be triggered right now on this product.');
    } else if (tx.product.handoffTimestampHistory.length > 1) {
        throw new Error('Unauthorized operation. This operation has already been triggered in the past for this product.');
    }
}
...
async function driverHandOff(tx) {
    if (tx.product.currentOwner === 'TRUCK' && tx.product.handoffTimestampHistory.length == 2) {
        ...
        let productRegistry = await getAssetRegistry(NS + '.Product');
        await productRegistry.update(tx.product);
    } else if (tx.product.currentOwner !== 'TRUCK') {
        throw new Error('Unauthorized operation. This function can only be triggered once by the truck driver.');
    } else if (tx.product.handoffTimestampHistory.length < 2) {
        throw new Error('Unauthorized operation. This operation cannot be triggered right now on this product.');
    } else if (tx.product.handoffTimestampHistory.length > 2) {
        throw new Error('Unauthorized operation. This operation has already been triggered in the past for this product.');
    }
...
async function managerHandOff(tx) {
    if (tx.product.currentOwner === 'WAREHOUSE' && tx.product.handoffTimestampHistory.length == 3) {
        ...
        let productRegistry = await getAssetRegistry(NS + '.Product');
        await productRegistry.update(tx.product);
    } else if (tx.product.currentOwner !== 'WAREHOUSE') {
        throw new Error('Unauthorized operation. This function can only be triggered once by the warehouse manager.');
    } else if (tx.product.handoffTimestampHistory.length < 3) {
        throw new Error('Unauthorized operation. This operation cannot be triggered right now on this product.');
    } else if (tx.product.handoffTimestampHistory.length > 3) {
        throw new Error('Unauthorized operation. This operation has already been triggered in the past for this product.');
    }
